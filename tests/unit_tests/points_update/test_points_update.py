...
def setup_data():
    ...
    # Clear after tests
    competitions[:] = [
        comp for comp in competitions if comp["name"] != "TestCompetition"
    ]
...
def test_points_update(client, setup_data):
    ...
    competition = [c for c in competitions if c["name"] == "TestCompetition"][
        0
    ]
    ...
    assert (
        ...
        == initial_competition_places - places_to_book
    )
    ...
    competitions.append(
        {
            ...
            "numberOfPlaces": initial_competition_places,
        }
    )
